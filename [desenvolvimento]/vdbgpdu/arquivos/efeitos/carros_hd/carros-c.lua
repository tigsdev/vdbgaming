...
function startCarPaintReflect()
		...
		local v = Settings2.var
		-- Create stuff
		grunShader = dxCreateShader ( "arquivos/efeitos/carros_hd/fx/car_refgrun.fx",1,v.renderDistance,false)
		geneShader = dxCreateShader ( "arquivos/efeitos/carros_hd/fx/car_refgene.fx",1,v.renderDistance,true)
		shatShader = dxCreateShader ( "arquivos/efeitos/carros_hd/fx/car_refgene.fx",1,v.renderDistance,true)

		myScreenSourceKocsi = dxCreateScreenSource( scx, scy)
		
		if grunShader and geneShader and shatShader and myScreenSourceKocsi then

			--outputConsole( "Started: Shader Car paint reflect.")
			
			setEffectV()
			
			addEventHandler ( "onClientHUDRender", getRootElement (), updateScreenKocsi )
			...
			-- Set texture
			textureVol = dxCreateTexture ( "arquivos/efeitos/carros_hd/images/smallnoise3d.dds" )
			
			dxSetShaderValue ( grunShader, "sRandomTexture", textureVol )
			dxSetShaderValue ( grunShader, "sReflectionTexture", myScreenSourceKocsi )
            
			dxSetShaderValue ( geneShader, "gShatt", false )
			dxSetShaderValue ( geneShader, "sRandomTexture", textureVol )
			dxSetShaderValue ( geneShader, "sReflectionTexture", myScreenSourceKocsi )
			
			dxSetShaderValue ( shatShader, "gShatt", true )
            dxSetShaderValue ( shatShader, "sRandomTexture", textureVol )
			dxSetShaderValue ( shatShader, "sReflectionTexture", myScreenSourceKocsi )			
...
function stopCarPaintReflect()
	...
	removeEventHandler ( "onClientHUDRender", getRootElement (), updateScreenKocsi )
	...
	geneShader = nil
	shatShader = nil
	destroyElement(myScreenSourceKocsi)
	destroyElement(textureVol)
	myScreenSourceKocsi = nil
	textureVol = nil
	killTimer(vehTimer)
	vehTimer = nil
...
function updateScreenKocsi()
	if myScreenSourceKocsi then
		dxUpdateScreenSource( myScreenSourceKocsi)
	end
end