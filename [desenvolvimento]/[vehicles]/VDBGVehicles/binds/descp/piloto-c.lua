...
function cruiseControlData()
	if (not cruiseEnabled) then
		...
		if ( speed < 0.1 ) then
			return exports.VDBGMessages:sendClientMessage ( "Vá mais devagar para ativar o piloto automático", 255, 255, 0 )
		end
		...
		exports["VDBGMessages"]:sendClientMessage("Piloto automático ativado.",0,255,0)
	else
		cruiseEnabled = false
		removeEventHandler("onClientRender",getRootElement(),cruiseControl)
		unbindKey("s","down",cruiseControlOff)
		...
		unbindKey("num_sub","down",decreaseCruiseSpeed)
		setControlState("accelerate",false)
		exports["VDBGMessages"]:sendClientMessage("Piloto automático desativado.",0,255,0)
	end
end
