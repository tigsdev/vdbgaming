...
setTimer ( function ( )
	doDraw_Graph = tobool ( exports.VDBGPhone:getSetting ( "usersettings_showspeedgraph" ) )
	doDraw_Speed = tobool ( exports.VDBGPhone:getSetting ( "usersettings_showspeedmeter" ) )
end, 500, 1 )
		...
		if ( vd[2] ) then
			...
			if ( isElement ( car ) ) then
				...
				if ( fuel2 <= 0 and getVehicleEngineState ( car ) ) then
					...
					exports['VDBGMessages']:sendClientMessage ( "Este veículo não tem gasolina!", 255, 0, 0 )
