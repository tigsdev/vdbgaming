...
addEventHandler ("onHandbrakeZoneChange", getRootElement(),
	function (zone)
		...
		if veh then
			...
			elseif zone == 3 then
				local handbrakeSound = playSound ("velocimetro1/freiodemao.wav")
		...
		if hand.show == true then
			...
			if veh then
				if getElementSpeed (veh) < 5 then
					dxDrawImage (hand.x,hand.y,65,275,"velocimetro1/Freidemaobg11.png")
					dxDrawImage(hand.x,hand.currentY,64,16,"velocimetro1/Freidemaobg1.png")
				end
			else
				showHandbrakeGui (false)
			end
...
function informatePlayerAboutHandbrake ()
	...
	if playerVeh then
		if getVehicleEngineState (playerVeh) == true then
			outputChatBox ("O freio de mão está puxado.", 255,168,0)
		end
	end
end
