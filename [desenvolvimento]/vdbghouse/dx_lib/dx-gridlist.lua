...
addEvent("onDxLibGridlistHouseClick",true)
addEvent("onDxLibHouseScrollMouse",true)
...
function dxLibCreateGridlist(gridlistElement,gridlistData,positionX,positionY,sizeX,sizeY,fontSize,emptyString,colorRed,colorGreen,colorBlue,alphaGridlist,coloredLabels,postGUI)
	if not getElementData(gridlistElement,"dxGridlistCreate") then
		...
		setElementData(gridlistElement,"house_dxGridlistSelected",0,false)
	...
	if type(gridlistData) == "table" then
		...
		if getElementData(gridlistElement,"house_dxGridlistSelected") > #gridlistData then
			setElementData(gridlistElement,"house_dxGridlistSelected",0,false)
		end
	else
	...
	if renderTarget then
		...
		setElementData(gridlistElement,"dxGridlistDataHover",false,false)
		local selectedRow = getElementData(gridlistElement,"house_dxGridlistSelected")
...
function onDxLibPlayerClick(button,state)
	if isCursorShowing() then
		if button == "left" and state == "down" then
			for i,dxGridlist in pairs(getElementsByType("dxlist_house")) do
				if getElementData(dxGridlist,"dxGridlistHover") then
					if getElementData(dxGridlist,"dxGridlistDataHover") then
						if getElementData(dxGridlist,"house_dxGridlistSelected") ~= tonumber(getElementData(dxGridlist,"dxGridlistDataHover")) then
						  setElementData(dxGridlist,"house_dxGridlistSelected",tonumber(getElementData(dxGridlist,"dxGridlistDataHover")),false)
						  triggerEvent("onDxLibGridlistHouseClick",getRootElement(),dxGridlist)
						end
					end
...
function onDxLibMouseScroll(keyPressed)
	if keyPressed == "mouse_wheel_up" then
		...
		triggerEvent("onDxLibHouseScrollMouse",getRootElement(),"up")
	elseif keyPressed == "mouse_wheel_down" then
		...
		triggerEvent("onDxLibHouseScrollMouse",getRootElement(),"down")
	end
end
...
function dxLibGridlistGetSelectedRowText(gridlistElement)
	if isElement(gridlistElement) or not getElementData(gridlistElement,"dxGridlistCreate") then
		if getElementData(gridlistElement,"house_dxGridlistSelected") and getElementData(gridlistElement,"house_dxGridlistSelected") ~= 0 then
			local data = getElementData(gridlistElement,"dxGridlistDatatable")
			local row = tonumber(getElementData(gridlistElement,"house_dxGridlistSelected"))
...
function dxLibGridlistGetSelectedRowNumber(gridlistElement)
	if isElement(gridlistElement) or not getElementData(gridlistElement,"dxGridlistCreate") then
		if getElementData(gridlistElement,"house_dxGridlistSelected") then
			local row = tonumber(getElementData(gridlistElement,"house_dxGridlistSelected"))
			if row then
				return row
			else
