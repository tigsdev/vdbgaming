MARKERS__ = {}
SHOPS = {
	{"Commerce",1643.39, -1514.31, 12.57,285},
	{"Temple",1041.32, -1017.38, 31.11},
...
addEventHandler("onResourceStart",resourceRoot,function()
	if SHOPS then
		...
		for k,v in ipairs ( SHOPS ) do
			...
			local marker = createMarker(x,y,z,"cylinder", 4, 255, 0, 0, 50 )
			local markermodels = createObject(2223, x,y,z+1)
			local markermodel = exports.VDBG3DTEXT:create3DText ( 'Tuning', { x, y, z+1 }, { 255, 0, 0 }, { nil, true },  { }, "Tuning", "LOJA")
			table.insert(MARKERS__,marker)
			setElementData(marker,"ROT",v[5])
			setElementData(marker,"ROTo",v[6])
			setElementData(markermodels, "carPickups", true)			
		end
...
function getElementSpeed(element,unit)
	...
	if (isElement(element)) then
		...
		outputDebugString("Não é um elemento. Não é possível obter velocidade")
		return false
	end
end
...
DIMS__ = math.random(65035,65535)
pPOS = {}
function playerTune(player,rot)
	if player then
		...
		if getPedOccupiedVehicleSeat(player) == 0 then
			if player then
				setElementData(player, "chatstate", true)
				setElementData(player, "hudstate", true)
				setElementData(player, "velocimetrostate", true)
				setElementData(player, "radarstate", true)
				...
				pPOS[player] = {x,y-2,z,rx,ry,rz,pDIM}
				setElementPosition(car,2387.1442871094,1053.4392089844,11.5)
...
addEventHandler("upgradeVehicle",root,function(id)
	if id then
		...
		if pVehicle then
			...
			showText("#FFFFFFSEU VEÍCULO FOI MODIFICADO COM SUCESSO!",source)
			fixVehicle(pVehicle)
		end
	end
end )
...
addEventHandler("unupgradeVehicle",root,function(id)
	if id then
		...
		if pVehicle then
			...
			setElementFrozen(pVehicle,true)
			showText("#FF0000UPGRADE FOI REMOVIDO",source)
		end
	end
end )
...
addEventHandler("paintjobVehicle",root,function(id)
	if id then
		...
		if pVehicle then
			...
			setElementFrozen(pVehicle,true)
			---
			showText("#ffff00ADESIVO FOI ADICIONADO",source)
			---
		end
	end
end )
...
addEventHandler("setTheColor",root,function(c)
	if c then
		...
		if pVehicle then
			...
			fixVehicle(pVehicle)
			showText(hex.."COR FOI MODIFICADA COM SUCESSO.",source)
		end
	end
end )
