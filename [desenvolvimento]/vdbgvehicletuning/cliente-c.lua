MAIN_MENU_NAME = "VDBG MODIFICAÇÕES"
function loadFonts ()
	FONT = guiCreateFont("FONT.ttf",14,true)
	FONT_SMALL = guiCreateFont("FONT.ttf",12)
	DX_TEXT_SMALL = dxCreateFont("FONT.ttf",12)
	doGarages()
end
...
addEventHandler("showCarTune",root,function(car)
	if car then
		...
		createMainMenu()
		saveUpgrades()
		SHOP_MUSIC = playSound("FASTVDBG.mp3",true)
...
addEventHandler("onClientRender", root,
    function()
   	if SURE_SHOW then
        ...
        dxDrawText("NÃO", xx + 248, yy + 180 , 498, 443, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("NÃO", xx + 248, yy + 180 , 498, 441, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("NÃO", xx + 248, yy + 180 , 496, 443, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("NÃO", xx + 248, yy + 180 , 496, 441, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("NÃO", xx + 248, yy + 180 , 497, 442, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
       
        dxDrawText("SIM", xx + 80, yy + 180, 342, 448, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("SIM", xx + 80, yy + 180, 342, 446, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("SIM", xx + 80, yy + 180, 340, 448, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("SIM", xx + 80, yy + 180, 340, 446, tocolor(0, 0, 0, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        dxDrawText("SIM", xx + 80, yy + 180, 341, 447, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
        
        dxDrawText(SURE_TEXT, xx + 10, yy + 145, 340, 141, tocolor(0, 0, 0, 255), 1.00, "default", "left", "center", false, false, true, false, false)
        dxDrawText(SURE_TEXT, xx + 10, yy + 145, 340, 141, tocolor(0, 0, 0, 255), 1.00, "default", "left", "center", false, false, true, false, false)
        dxDrawText(SURE_TEXT, xx + 10, yy + 145, 340, 141, tocolor(0, 0, 0, 255), 1.00, "default", "left", "center", false, false, true, false, false)
        dxDrawText(SURE_TEXT, xx + 10, yy + 145, 340, 141, tocolor(0, 0, 0, 255), 1.00, "default", "left", "center", false, false, true, false, false)
        dxDrawText(SURE_TEXT,xx + 10, yy + 145, 340, 141, tocolor(255, 255, 255, 255), 1.00, "default", "left", "center", false, false, true, false, false)

        
        dxDrawText("VOCÊ TEM CERTEZA?", xx + 10, yy + 9, 537, 269, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
	...
	if gui then
		...
		if price then
			...
			if num > 2 then
				...
				if num == 2 then
					if string.lower(text) == "voltar" then
						backMenu()
						modifyCam()
					else
						killMenu()
...
function getTheColor(item)
	...
	for k,v in ipairs ( ___MENUS['COR DO VEÍCULO'] ) do
...
function onItemBuy(item,price,upid)
	if item and price then
		...
		local menu = _MENU_[1].text
		upg = false
		for k,v in ipairs ( ___MENUS['TUNAR VEÍCULO'] ) do
			if string.lower(menu) == string.lower(v[1]) then
				upg = true
			end
		end
		if menu ~= 'TUNAR VEÍCULO' then
			if upg == true then
				if upid > 3 then
					local ex = doesUpgradeExist(upid)
					if not ex then
						if checkMoney(price) then
							showSure("VOCÊ QUER COMPRAR "..item.." POR R$"..price..",00 ?","buyUpgrade",item,upid)
						end
					else
						showSure("VOCÊ JÁ TEM: "..item..", DESEJA REMOVÊ-LO?","removeUpgrade",item,upid)
					end
				else
					if checkMoney(price) then
						showSure("VOCÊ QUER COMPRAR "..item.." POR R$"..price..",00 ?","buyPaintjob",item,upid)
					end
				end
			end
		end
		if menu == 'TUNAR VEÍCULO' then
			if string.lower(item) == string.lower('HIDRÁULICA') then
				local ex = doesUpgradeExist(1087)
				if not ex then
					if checkMoney(price) then
						showSure("VOCÊ QUER COMPRAR "..item.." POR R$"..price..",00 ?","buyUpgrade",item,1087)
					end
				else
					showSure("VOCÊ JÁ TEM: "..item..", DESEJA REMOVÊ-LO?","removeUpgrade",item,1087)
				end
			elseif string.lower(item) == string.lower('PLATE NUMBER') then
				showSure("VOCÊ QUER MUDAR O SEU "..item.." POR R$ "..price..",00 ?","changePlatenumber",item)
			end
		end
		------
		if menu == MAIN_MENU_NAME then
			if item == 'REPARAR VEÍCULO' then
				if checkMoney(price) then
				showSure("VOCÊ QUER REPARAR SEU VEÍCULO POR R$ "..price..",00 ?","buyVehicleFix",item,upid)
				end
			elseif item == 'COR DO VEÍCULO' then
				if checkMoney(price) then
					showSure("VOCÊ QUER MUDAR A COR DO SEU VEÍCULO POR R$ "..price..",00 ?","buyVehicleColor",item,upid)
				end
			end
		end
	end
...
function setUpMenu(MENU,main,back)
	...
	if  ___MENUS[MENU] then
		for k,v in ipairs( ___MENUS[MENU] ) do
			...
			if MENU == "TUNAR VEÍCULO" and price == false then
...
function LoadPaintjobs()
	if PLAYER_VEHICLE then
		addItem("ADESIVO 1",2000,false,false,0)
		addItem("ADESIVO 2",2000,false,false,1)
		addItem("ADESIVO 3",2000,false,false,2)
		addItem("SEM ADESIVO",0,false,false,3)
	end
end
...
function ()
	...
	for i,pickup in pairs(getElementsByType"object") do
		if (getElementData(pickup, "carPickups") == true) then
