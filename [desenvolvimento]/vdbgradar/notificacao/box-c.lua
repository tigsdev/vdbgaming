...
function showBox(value, str)
	...
	if str and type(str) == "string" and string.len(str) > 0 then
		box = true
		if value == "info" then 
			showTipBox (str,"notification/img/info.png")
			outputConsole ("[INFO]" .. str)
		elseif value == "error" then 
			showTipBox (str,"notification/img/error.png")
			outputConsole ("[ERRO]" .. str)
		elseif value == "aviso" then 
			showTipBox (str,"notification/img/warning.png")
			outputConsole ("[AVISO]" .. str)
		elseif value == "admin" then 
			showTipBox (str,"notification/img/admin.png")
			outputConsole ("[ADMIN]" .. str)
		end
	end
	--else
	--	return false
...
function showTipBox (str,path)
	if str then
		if path == nil then
			path = "notification/img/info.png"
		end
	...
	function ()
		if tipBox.show == true and tipBox.string then
			...
			if width then
				...
				local textWidth,textHeight = 400,100
				dxDrawImage (x,y,width,256,tipBox.path,0,0,0,tocolor(255,255,255),true)
				dxDrawText (tipBox.string,textX,textY,textX+textWidth,textY+textHeight,tocolor(255,255,255,255),1,"default-bold","center","center", false, false, false, true, false)
			end
		end
	end
)
