...
addEvent ( "VDBGAmmunation:Module->Training:onClientHitMarker", true )
addEventHandler ( "VDBGAmmunation:Module->Training:onClientHitMarker", root, function ( s )
	if isTraining then return end
	local w = getPedWeapon ( localPlayer )
	local d = getElementData ( localPlayer, "VDBGSQL:WeaponStats" )
	if not ( d[allowedWeapons[w]] ) then 
		return exports.VDBGMessages:sendClientMessage ( "Esta arma não é permitida a ser treinados.", 255, 255, 0 )
	end
	if ( d[allowedWeapons[w]] > 90 ) then
		return exports.VDBGMessages:sendClientMessage ( "Esta arma tem a máxima eficiência!", 255, 255, 0 )
	end
...
function Training_OnRender ( )
	...
	dxDrawBorderedText ( "Habilidades", windowX, windowY-(rsy*25), windowX+windowW, windowY+windowH, tocolor(255,255,255,255), rsy*2, "beckett", "center", "top", false, false, false, false, false )
	
	dxDrawText ( "Aqui no centro de treinamento da armas da Ammu-nation, você pode aumentar suas habilidades de arma. A maioria das armas (Mostrado na F3 -> Statisticas) têm um ranking arma de 0/100. Aqui, você pode aumentar as habilidades das armas para que eles possam atirar mais rápido, você pode ter duelo, e outros.", windowX + ( rsx * 20 ), windowY + ( rsy * 40 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*50), tocolor ( 255, 255, 255, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawText ( "Selecione: N/A\nHabilidade: 0/100\nSeção custa: R$1,200", windowX + ( rsx * 20 ), windowY + ( rsy * 225 ), ( windowX+windowW)-(rsx * 20 ), (windowY+windowH)-(rsy*280), tocolor ( 255, 200, 65, 255 ), rsy*1.1, "default-bold", "left", "top", false, true )
	dxDrawRectangle ( windowX+(rsx*20), info.rect.y, windowW-(rsx*40), rsy*35, tocolor ( 0, 0, 0, 150 ) )
	
	dxDrawText ( "Treinar agora",  info.accept.x, info.accept.y, windowX+(windowW-(rsx*40)), info.accept.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	dxDrawText ( "Sair",  info.exit.x, info.exit.y, windowX+(windowW-(rsx*40)), info.exit.y+(rsy*35), tocolor(255,255,255,255), rsy*1.2, "default-bold", "center", "center" )
	...
	if ( k == "space" ) then
		...
		elseif ( selectedOption == 0 ) then
			...
			if ( not allowedWeapons [ getPedWeapon ( localPlayer ) ] ) then
				return exports.VDBGMessages:sendClientMessage ( "Esta arma não é permitida" )
			end
			
			if ( getPlayerMoney ( ) < 1200 ) then
				return exports.VDBGMessages:sendClientMessage ( "Você não tem R$ 1.200 para treinar com esta arma", 255, 255, 0 )
			end
...
function Training_StartTraining ( )
	...
	triggerServerEvent ( "VDBGAmmunation:Modules->Training:TakePlayerMoney", localPlayer, localPlayer, 1200 )
end
...
function Training_UserStats ( )
	...
	dxDrawText ( "Status", winX, winY+(rsy*5), winX+winW, winY+winH, tocolor ( 255, 255, 255, 255 ), rsy*2, "default-bold", "center" )
	dxDrawText ( "Matous: "..tostring(training.TargetShot), winX+(rsx*25), winY+(rsy*50), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
	dxDrawText ( "Meta: "..tostring(training.RequiredPeds), winX+(rsx*25), winY+(rsy*90), 0, 0, tocolor ( 255, 160, 0, 255 ), rsy*1.5, "default-bold" )
		...
		if ( training.TargetShot == training.RequiredPeds ) then
			...
			setTimer ( function ( )
				...
				if ( trainingWeapon ~= getPedWeapon ( localPlayer ) ) then
					return exports.VDBGMessages:sendClientMessage ( "Você mudou sua arma .... Sem habilidades adicionadas.", 255, 0, 0 )
				end
				
				local w = getPedWeapon ( localPlayer )
				local d = getElementData ( localPlayer, "VDBGSQL:WeaponStats" )
				d[allowedWeapons[w]] = d[allowedWeapons[w]] + 10
				setElementData ( localPlayer, "VDBGSQL:WeaponStats", d )
				exports.VDBGMessages:sendClientMessage ( "Você UPOU! novo nível: "..d[allowedWeapons[w]].."/100", 0, 255, 0 )
				triggerServerEvent ( "VDBGAmmunation:Modules->Training:onClientLevelWeaponUp", localPlayer, allowedWeapons[w], d[allowedWeapons[w]], w )
