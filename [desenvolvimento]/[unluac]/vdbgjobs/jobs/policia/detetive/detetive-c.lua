...
function detective.startCase ( skin, x, y, z )
	if ( getElementData ( localPlayer, "Job" ) ~= "Detetive" or table.len ( runningCases ) >= 5 ) then
		return
	end
	...
	setElementData ( r.ped, "VDBGJobs->Detective:InvestigationID", caseid )
	
	r.objects = { }
	r.tempCols = { }
	r.blip = createBlip ( x, y, z, 25 )
	r.col = createColSphere(x, y, z, 4.5)
	setElementData ( r.col, "VDBGJobs->Detective:InvestigationID", caseid )
	...
	local msg = "Alguém foi morto no "..getZoneName(x,y,z)..", "..getZoneName(x,y,z,true).." area. Vá até a cena do crime (o ícone de dados) e resolva o caso"
	outputChatBox ( "#d9534f[D.P] #FFFFFF"..msg, 255, 255, 255, true )
	exports.VDBGPolice:outputDispatchMessage ( msg )
end

function detective.startInvestigation ( p )
	if ( p ~= localPlayer ) then return end
	if ( isPedInVehicle ( p ) ) then return end
	local id = getElementData ( source, "VDBGJobs->Detective:InvestigationID" )
	...
	outputChatBox ( "#d9534f[D.P] #FFFFFFPesquisar a área em busca de pistas do assassino ...", 255, 255, 255, true)
	...
	for i=1, 15 do
		...
		setElementData ( s, "VDBGJobs->Detective:InvestigationObject", obj )
		setElementData ( s, "VDBGJobs->Detective:InvestigationID", id )
...
function detective.onPlayerGatherEvidence ( p )
	...
	local id = getElementData ( source, "VDBGJobs->Detective:InvestigationID" )
	local o = getElementData ( source, "VDBGJobs->Detective:InvestigationObject" )
	...
	if ( runningCases[id].evidence < 4 ) then
		outputChatBox ( "#d9534f[D.P] #FFFFFFEsta parece ser uma pista que conduz a um suspeito. Continuar pesquisando até que possamos encontrar quem fez isso", 255, 255, 255, true)
	else
...
function detective.stopCase ( id )
	...
	if ( isElement ( runningCases[id].blip ) ) then destroyElement ( runningCases[id].blip ) end
	triggerServerEvent ( "VDBGJobs:Modules->Detective:onClientFinishCase", localPlayer, id )
	runningCases[id] = nil
end

addEvent ( "VDBGJobs:Modules->Detective:onStartCase", true )
addEventHandler ( "VDBGJobs:Modules->Detective:onStartCase", root, function ( p, x, y, z, s )
