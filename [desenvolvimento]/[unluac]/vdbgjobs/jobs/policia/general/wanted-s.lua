function giveWantedPoints ( p, amount )
	if ( p and isElement ( p ) and tonumber ( amount ) ) then
		...
		local wl = tonumber ( getElementData ( p, "WantedPoints" ) or 0 )
...
addEventHandler ( "onPlayerDamage", root, function ( p, weap, loss )
	if ( isElement ( p ) and p ~= source ) then
		...
		if ( getElementData ( source, "VDBGEvents:IsPlayerInEvent" ) or getElementData ( p, "VDBGEvents:IsPlayerInEvent" ) ) then
			return
		end
		...
		if t then
			...
			if weap then
				if ( t == "Emergencia" and getElementHealth ( source ) < 100 and weap == 14 ) then
					if ( getElementData ( p, "Job" ) == "Medico" ) then
						return
					end
				elseif ( exports['VDBGPlayerFunctions']:isTeamLaw ( t ) and ( weap == 3 or weap == 23 ) and getPlayerWantedLevel ( source ) > 0 ) then
					return
				end
			end
		end
...
setTimer ( function ( )
	...
	for i, v in ipairs ( getElementsByType ( 'player' ) ) do 
		if ( not getElementData ( v, "VDBGJobs:ArrestingOfficer" ) and not getElementData ( v, "isPlayerJailed" ) ) then
		...
		if job  then
			...
			if ( w_l ~= 0 and w_l > max_ ) then
				outputChatBox ( "#d9534f[D.P] #FFFFFFVocê está sendo procurado por ter um alto nível de procurado.", v, 255, 255, 255, true )
				resignPlayer ( v )
			end
		end
	end
