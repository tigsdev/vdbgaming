...
CMD_LASERCOLOR = "cor" -- command to change laser color
...
function ToggleLaserEnabled(cmd)
	if ( not isPlayerVIP ( ) ) then exports.VDBGMessages:sendClientMessage("Este comando é só para VIP.",255,255,255) return SetLaserEnabled(localPlayer,false) end
	player = localPlayer
	if IsLaserEnabled(player) == false then	
		SetLaserEnabled(player, true)
		exports.VDBGMessages:sendClientMessage ( "Seu lazer foi ativado", 0, 255 ,0 )
	else
		SetLaserEnabled(player, false)
		exports.VDBGMessages:sendClientMessage ( "Seu lazer foi desativado.", 255, 255, 0 )
	end
end
...
function ChangeLaserColor ( cmd )
	...
	exports.cpicker:openPicker ( "LaserColorPicker", hex, "Laser Color" )
end
