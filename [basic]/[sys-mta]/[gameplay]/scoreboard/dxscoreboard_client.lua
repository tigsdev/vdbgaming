...
seperationSpace = 120 -- the space between top/bottom screen and scoreboard top/bottom in pixels
...
addEventHandler( "onClientResourceStart", getResourceRootElement( getThisResource() ), 
	function ( resource )
		...
		cServerInfo = tocolor( 66, 139, 202, 255 )
...
function doDrawScoreboard( rtPass, onlyAnim, sX, sY )
	if #scoreboardColumns ~= 0 then
				...
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							local playerName = getElementData(player, "AccountData:Name")
		...
		if scoreboardDrawn then
			...
			local backGroundHeight =  s(3)+s(3)+s(5)+s(5)+dxGetFontHeight( fontscale(serverInfoFont, s(1)), serverInfoFont )+dxGetFontHeight( fontscale(columnFont, s(0.8)), columnFont )
			dxDrawRectangle(topX,topY,scoreboardDimensions.width,backGroundHeight,tocolor(0,0,0,190))
			local y = topY+s(5)
			if serverInfo.server and showServerInfo then
				dxDrawText( "VDBGaming - www.vdbg.org", topX+s(5)+s(1), y+s(1), topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), tocolor(0,0,0,190), fontscale(serverInfoFont, s(1)), serverInfoFont, "left", "top", false, false, drawOverGUI )
				dxDrawColorText( "#428bcaVDB#ffffffGaming - www.vdbg.org", topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), tocolor(255, 255, 255, 255), fontscale(serverInfoFont, s(1)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
			if showServerInfo and serverInfo.players then
				local players = getElementsByType( "player" )
				local text = "Online: " .. tostring( #players ) .. "/" .. serverInfo.players
				local textWidth = dxGetTextWidth( text, fontscale(serverInfoFont, s(1)), serverInfoFont )
				dxDrawText( text, topX+scoreboardDimensions.width-s(5)-textWidth+s(1), y+s(1), topX+scoreboardDimensions.width-s(5), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), tocolor(0,0,0,190), fontscale(serverInfoFont, s(1)), serverInfoFont, "left", "top", false, false, drawOverGUI )
				dxDrawText( text, topX+scoreboardDimensions.width-s(5)-textWidth, y, topX+scoreboardDimensions.width-s(5), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), tocolor(255, 255, 255, 255), fontscale(serverInfoFont, s(1)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
			if (serverInfo.server or serverInfo.players) and showServerInfo then y = y+dxGetFontHeight( fontscale(serverInfoFont, s(1)), serverInfoFont ) end
			
			local textLength = dxGetTextWidth( "Segure RMB para rolar", fontscale(rmbFont, s(0.75)), rmbFont )
			...
			textLength = dxGetTextWidth( "configurar...", fontscale(sbFont, s(sbFontScale)), sbFont )
			...
			for key, column in ipairs( scoreboardColumns ) do
				...
				if sortBy.what == column.name then
					...
					dxDrawText( column.friendlyName or "-", topX+x+s(1+9), y+s(1), 	topX+x+s(1+column.width), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(0.8)), columnFont, "left", "top", true, false, drawOverGUI )
					dxDrawText( column.friendlyName or "-", topX+x+s(9), 	y, 	topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 255, 255, 255, 255 ), fontscale(columnFont, s(0.8)), columnFont, "left", "top", true, false, drawOverGUI )
					...
					local _, _, _, a = fromcolor( cHeader )
					dxDrawText( column.friendlyName or "-", topX+x+s(1), 	y+s(1), 	topX+x+s(1+column.width), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(0.9)), columnFont, "left", "top", true, false, drawOverGUI )
					dxDrawText( column.friendlyName or "-", topX+x, 	y, 	topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor(255, 255, 255, 255), fontscale(columnFont, s(0.9)), columnFont, "left", "top", true, false, drawOverGUI )
				end
			...
			while ( index < firstVisibleIndex+maxPerWindow and scoreboardContent[index] ) do
				...
				if element and isElement( element ) and getElementType( element ) == "team" then
					dxDrawRectangle( topX, y, scoreboardDimensions.width, dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), tocolor(0,0,0,190), drawOverGUI )
					...
					if checkCursorOverRow( rtPass, topX+s(5), topX+scoreboardDimensions.width-s(5), y, y+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ) ) then
						dxDrawRectangle( topX, y, scoreboardDimensions.width, dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), cHighlight, drawOverGUI )
					end
					for key, column in ipairs( scoreboardColumns ) do
				...
				elseif element and isElement( element ) and getElementType( element ) == "player" then
					...
					if element == getLocalPlayer() then
						dxDrawRectangle( topX, y, scoreboardDimensions.width, 1.5*dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), cSelection, drawOverGUI )
					end
					-- Highlight the the row on which the cursor lies on
					if checkCursorOverRow( rtPass, topX, topX+scoreboardDimensions.width, y, y+1.5*dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ) ) then
						dxDrawRectangle( topX, y, scoreboardDimensions.width, 1.5*dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), cHighlight, drawOverGUI )
					end
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "id" then
							dxDrawRectangle(topX,y,topX+x+s(1+column.width),(1.5*dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont )),tocolor(0,0,0,230))
						end
						if content then
							if serverInfo.allowcolorcodes and type( content ) == "table" and column.name == "name" then
								local playerName = content[1]
							...
							elseif column.name == "country" then	
								...
								if not fileExists(countryImage) then
									countryImage = ':admin/client/images/flags/unknown.png'
								end
...
function calculateHeight()
	...
	local index = firstVisibleIndex
	local height = s(5)
	if (serverInfo.server or serverInfo.players) and showServerInfo then height = height+dxGetFontHeight( fontscale(serverInfoFont, s(1)), serverInfoFont ) end
	if (serverInfo.gamemode or serverInfo.map) and showGamemodeInfo then height = height+dxGetFontHeight( fontscale(serverInfoFont, s(1)), serverInfoFont ) end
	height = height+s(5)+dxGetFontHeight( fontscale(columnFont, s(0.8)), columnFont )+s(3)+s(3)
	while ( index < firstVisibleIndex+maxPerWindow and scoreboardContent[index] ) do
		local element = scoreboardContent[index]["__SCOREBOARDELEMENT__"]
		if element and isElement( element ) and getElementType( element ) == "team" then
			height = height + dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont )
		else
			height = height + (1.5*dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ))
		end
		index = index + 1
	end
	return height
