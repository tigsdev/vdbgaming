function openBanWindow ( acc )
	...
	Ban.window = guiCreateWindow((sx/2-250/2), (sy/2-270/2), 250, 270, "Contas banidas", false)
	guiWindowSetSizable(Ban.window, false )
	Ban.account = guiCreateLabel(20, 37, 315, 23, "Conta: "..tostring(acc), false, Ban.window)
	Ban.misc_1 = guiCreateLabel(19, 102, 75, 22, "Dia:", false, Ban.window)
    Ban.misc_2 = guiCreateLabel(19, 134, 82, 22, "Mes:", false, Ban.window)
	Ban.misc_3 = guiCreateLabel(19, 166, 82, 22, "Ano:", false, Ban.window)
	Ban.misc_4 = guiCreateLabel(19, 198, 82, 22, "Rasão:", false, Ban.window)
	Ban.cancel = guiCreateButton(19, 230, 82, 33, "Cancelar", false, Ban.window)
	Ban.ban = guiCreateButton(106, 230, 82, 33, "Banir", false, Ban.window) 
	...
	Ban.unbanLabel = guiCreateLabel(19, 61, 316, 22, "Desbanida em: "..table.concat({guiGetText(Ban.unbanDay),guiGetText(Ban.unbanMonth),guiGetText(Ban.unbanYear)},"/").." ("..getDaysToDate(data)..")", false, Ban.window)
...
function onClientBanButtonClick ( )
	...
	elseif ( source == Ban.ban ) then
		...
		if ( days:sub ( 0, 1 ) == "-" ) then -- check if date is passed
			return outputChatBox ( "Data inválida, ou já passou." )
		end

		triggerServerEvent ( "VDBGAdmin:Modules->Banner:onAdminBanClient", localPlayer, account, tonumber(data.day), tonumber(data.month), tonumber(data.year), guiGetText ( Ban.reason ), days )
		destroyBanWindow ( )
	end
end
...
function onClientBanEditEdit ( )
	...
	guiSetText ( Ban.unbanLabel, "Desbanida em: "..fDate.." ("..getDaysToDate(data)..")" )
end
