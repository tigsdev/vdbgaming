...
addEvent ( "VDBGAdministration:AccountManager:onClientOpenWindow", true )
addEventHandler ( "VDBGAdministration:AccountManager:onClientOpenWindow", root, function ( d )
...
function amOnGUIClick ( )
	if ( source == aManager.accounts ) then
		...
		if ( accounts_invalid [ t ] ) then
			exports.VDBGMessages:sendClientMessage("Invalid - "..tostring(accounts_invalid[t].reason), 255, 0, 0 )
		end
	...
	elseif ( source == aManager.button['DeleteAccount'] ) then
		...
		askConfirm ( "Tem certeza que quer deletar essa conta?\n"..tostring(name).."?", amOnConfirmCallback )
	elseif ( source == aManager.button['ExecuteSave'] ) then
		askConfirm ( "Tem certeza que quer salvar o banco de dados ? da LAG.", function ( x )
			if not x then return end
			triggerServerEvent ( "VDBGAdmin:aManager:ExecuteServerSave", localPlayer )
		end )
		
	elseif (source == aManager.button['BanAccount'] ) then
		local row,_ = guiGridListGetSelectedItem ( aManager.accounts )
		if ( row == -1 ) then return outputChatBox ( "Você não selecionou uma conta", 255, 0, 0 ) end
...
function amOnGUIChange ( )
	if ( source == aManager.search ) then
		...
		local r = guiGridListAddRow ( aManager.accounts )
		guiGridListSetItemText ( aManager.accounts, r, 1, "Validadas", true, true )
		guiGridListSetItemColor ( aManager.accounts, r, 1, 0, 255, 0 )
		...
		guiGridListSetItemText ( aManager.accounts, guiGridListAddRow ( aManager.accounts ), 1, "", true, true )
		local r = guiGridListAddRow ( aManager.accounts )
		guiGridListSetItemText ( aManager.accounts, r, 1, "não válidadas", true, true )
		guiGridListSetItemColor ( aManager.accounts, r, 1, 255, 0, 0 )
...
function amOnConfirmCallback ( c )
	if c then
		triggerServerEvent ( "VDBGAdmin:amManager:removeAccountFromHistory", localPlayer, name )
	end
end
