eventStartFunctions[123213] = _G['StartOneInTheChamber']
StartOneInTheChamber = nil


events[123213] = { 
	name 						= 'Um tiro, um ganhador.',-- The event name
...
function StartOneInTheChamber ( ) 
	for i, v in pairs ( EventCore.GetPlayersInEvent ( ) ) do
		...
		exports.VDBGMessages:sendClientMessage ( "Use sua munição com atenção! Você vai ter uma bala para cada morte", v, 255, 255, 255 )
	end
	...
	addEventHandler ( "VDBGEvents:onEventEnded", root, thisEventFunctions.OnEventEnded )
end
...
function thisEventFunctions.OnEventEnded ( x )
	if ( x.name == events[2].name ) then
		...
		removeEventHandler ( "VDBGEvents:onEventEnded", root, thisEventFunctions.OnEventEnded )
	end
end
