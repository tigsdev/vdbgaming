...
addEvent ( "VDBGSpawners:onStaffOpenEditor", true )
addEventHandler ( "VDBGSpawners:onStaffOpenEditor", root, function ( data ) 
...
function adminEditorClicking ( )
	...
	elseif ( source == btnSave ) then
		triggerServerEvent ( "VDBGSpawners:onAdminEditSpawnerList", localPlayer, guiGetText ( spawnerData ) )
	end
end
