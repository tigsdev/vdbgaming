...
local window = guiCreateWindow( ( sx / 2 - ( rsx*354 ) / 2 ), ( sy / 2 - (rsy*400) / 2 ), (rsx*354), (rsy*400), "Carros VDBG", false)
local vehList = guiCreateGridList((rsx*9), (rsy*26), (rsx*335), (rsy*315), false, window)
local btnSpawn = guiCreateButton((rsx*13), (rsy*346), (rsx*149), (rsy*43), "Pegar", false, window)
local btnClose = guiCreateButton((rsx*195), (rsy*346), (rsx*149), (rsy*43), "Cancelar", false, window)
guiWindowSetSizable(window, false)
guiSetVisible ( window, false )
guiGridListAddColumn(vehList, "Carro", 0.9)
local marker = nil
addEvent ( "VDBGSpawners:ShowClientSpawner", true )
addEventHandler ( "VDBGSpawners:ShowClientSpawner", root, function ( cars, mrker )
	...
	if ( not guiGetVisible ( window ) ) then 
		...
		marker = mrker
		job = getElementData ( marker, "VDBGVehicles:JobRestriction" )
		guiGridListSetItemText ( vehList, guiGridListAddRow ( vehList ), 1, "Carros para Jogadores", true, true )
		...
		if ( exports.VDBGVIP:isPlayerVIP ( ) ) then
			local level = exports.VDBGVIP:getVipLevelFromName ( getElementData ( localPlayer, "VIP" ) )
			if ( level and level > 0 and VipVehicles [ level ] and #VipVehicles [ level ] > 0 ) then
				guiGridListSetItemText ( vehList, guiGridListAddRow ( vehList ), 1, "Carros VIP", true, true )
...
function spawnClickingFunctions ( )
	...
	elseif ( source == btnSpawn ) or getKeyState( "space" ) == true then
		...
		if ( row == -1 ) then
			return outputChatBox ( "d9534f[VDBG.ORG] #ffffffSelecione um veículo a pegar.", 255, 255, 0 )
		end
		
		local id = guiGridListGetItemData ( vehList, row, 1 )
		triggerServerEvent ( "VDBGSpawners:spawnVehicle", localPlayer, id, marker, true )
		closeWindow ( localPlayer )
	end
end 
...
addEvent ( "VDBGSpawners:CloseWindow", true )
addEventHandler ( "VDBGSpawners:CloseWindow", root, closeWindow )
