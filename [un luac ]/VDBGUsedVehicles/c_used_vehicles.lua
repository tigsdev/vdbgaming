...
addEventHandler ( "onClientResourceStart", resourceRoot, function ( )
	setTimer ( function ( )
		local makeBlips = exports.vdbgphone:getSetting ( "usersetting_display_usedvehicleshopblips" );
...
function createWindow ( ) 
	...
	triggerServerEvent ( "VDBGShops->UsedVehicles->onClientRequestUsedList", localPlayer );
	...
	showCursor ( true );
	exports.vdbgmessages:sendClientMessage ( "Please wait as the used vehicle shop is loaded", 255, 140, 0 );
...
function onGuiClick ( )
	...
	elseif ( source == gui.viewDesc ) then 
		...
		if ( r == -1 ) then
			return exports.vdbgmessages:sendClientMessage ( "You must select a vehicle for this action!", 200, 100, 130 );
		end
	...
	elseif ( source == gui.purchase ) then 
		...
		askConfirm ( "Are you sure you would like to purchase this vehicle?", function ( x, id )
			if ( x ) then 
				triggerServerEvent ( "VDBGShops->UsedVehicles->onPlayerTryBuyVehicle", localPlayer, id );
			end
		end, id );
	...
	if ( sell ) then 
		...
		elseif ( source == sell.sell ) then 
			...
			if ( row == -1 ) then 
				return exports.vdbgmessages:sendClientMessage ( "You must one of your vehicles for this action!", 200, 100, 130 );
			end 
			...
			if ( tostring ( data.Visible ) == "1" ) then 
				return exports.vdbgmessages:sendClientMessage ( "You must hide your vehicle before selling it", 200, 100, 130 );
			elseif ( tostring ( data.Impounded ) == "1" ) then 
				return exports.vdbgmessages:sendClientMessage ( "Your vehicle cannot be impounded when selling it.", 200, 100, 130 );
			elseif ( not price or math.floor ( price ) ~= price ) then 
				return exports.vdbgmessages:sendClientMessage ( "Invalid price - must be an integer (whole number)", 200, 100, 130 );
			elseif ( price < 1000 or price > 1000000000 ) then 
				return exports.vdbgmessages:sendClientMessage ( "Price must be $1,000 - $1,000,000,000", 200, 100, 130 );
			elseif ( string.len ( desc ) < 10 or string.len ( desc ) > 2000 )  then 
				return exports.vdbgmessages:sendClientMessage ( "Description must be 10-2000 characters", 200, 100, 130 );
			end 
			
			triggerServerEvent ( "VDBGUsedVehicles->SellVehicle->OnPlayerTrySellVehicle", localPlayer, data.VehicleID, price, desc );
...
addEvent ( "VDBGShops->UsedVehicles->onServerSendClientList", true );
addEventHandler ( "VDBGShops->UsedVehicles->onServerSendClientList", root, function ( list, mine )
	vList = list;
	myvehicles = mine
...
addEvent ( "VDBGUsedVehicles->Interface->SetInterfaceOpen", true )
addEventHandler ( "VDBGUsedVehicles->Interface->SetInterfaceOpen", root, function ( b )
